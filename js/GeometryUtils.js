...
THREE.GeometryUtils = {
	...
	randomPointInTriangle: function () {
		...
		return function ( vectorA, vectorB, vectorC, normalA, normalB, normalC ) {
			...
			normal.copy( normalB );
			normal.multiplyScalar( b );

			point2.add( normal );

			normal.copy( normalC );
			normal.multiplyScalar( c );
	...
	randomPointsInBufferGeometry: function ( geometry, n ) {
		...
		for ( i = 0; i < n; i ++ ) {
			...
			nA.set( normals[ index * 9 + 0 ], normals[ index * 9 + 1 ], normals[ index * 9 + 2 ] );
			nB.set( normals[ index * 9 + 3 ], normals[ index * 9 + 4 ], normals[ index * 9 + 5 ] );
			nC.set( normals[ index * 9 + 6 ], normals[ index * 9 + 7 ], normals[ index * 9 + 8 ] );
			result = THREE.GeometryUtils.randomPointInTriangle( vA, vB, vC, nA, nB, nC );
			pResult[i] = result[0];
			nResult[i] = result[1];
